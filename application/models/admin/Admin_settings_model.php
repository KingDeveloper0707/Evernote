...
class Admin_settings_model extends CI_Model
{
	...
	public function get_info_users()
	{
		...
		$this->db->select(' ci_users.id AS userid, ci_users.username, ci_users.email,ci_users.photo, ci_users.company, ci_users.position_title, ci_users.is_active, ci_users.created_at, COUNT(ci_templates.user_id) AS note_counts, MAX(ci_templates.updated_at) AS note_updated');
	...
	//get all companies counts
	public function get_all_counts_companies()
	{
		$counts = $this->db->count_all_results('ci_companies');
		return $counts;
	}

	//get all note types counts
	public function get_all_counts_notetypes()
	{
		$counts = $this->db->count_all_results('ci_notetypes');
		return $counts;
	}
	...
	//Get all notetype list
	public function get_all_notetype_list()
	{
	...
	//Delete Company
	public function del_company($ids)
	{

		$this->db->where('id', $ids);
		$this->db->delete('ci_companies');
	...
	//Delete Note type
	public function del_notetype($ids)
	{

		$this->db->where('id', $ids);
		$this->db->delete('ci_notetypes');
	...
	//Rename companies
	public function rename_companies($id, $data)
	{
	...
	//Rename notetypes
	public function rename_notetypes($id, $data)
	{

		$this->db->where('id', $id);
		$this->db->update('ci_notetypes', $data);
		return true;
	}
